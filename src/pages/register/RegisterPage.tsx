...
import { setAccessToken, setCaptchaValue, setDob } from "../../api/auth/authSlice";
...
const RegisterPage = () => {
      ...
      if (!countryDropdown.selectedValue) {
        try {
          ...
          if (data && data.status === "success" && data.country) {
            const found = countryOptions.find((name) => name.toLowerCase() === data.country.toLowerCase());
            ...
            console.error("Failed to get country from API response:", data.message || "Unknown error");
          }
        } catch (error) {
  ...
  }, [genderDropdown.selectedValue, countryDropdown.selectedValue, dob, captchaValue, setValue, trigger]);
  ...
  return (
    <div className="relative z-30 w-full h-full flex flex-col items-center justify-center px-4 pt-[3.375rem]">
      <div className="text-center mx-auto w-full max-w-[65rem]">
        ...
        {/* Registration Form */}
        <form className="text-start max-w-[45rem] mx-auto" onSubmit={handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[1.875rem]">
            ...
            <div>
              ...
              <input type="text" {...register("firstName")} placeholder="John" autoComplete="off" className={`w-full p-4 bg-inputBg text-white placeholder:text-subTitle rounded-lg focus:outline-none focus:border ${errors.firstName ? "border-red-500" : "focus:border-primary-main"}`} />
            ...
            <div>
              ...
              <input type="text" {...register("lastName")} placeholder="Doe" autoComplete="off" className={`w-full p-4 bg-inputBg text-white placeholder:text-subTitle rounded-lg focus:outline-none focus:border ${errors.lastName ? "border-red-500" : "focus:border-primary-main"}`} />
          ...
          <div className="mt-[1.875rem]">
            ...
            <input type="email" {...register("email")} placeholder="johnexample@gmail.com" autoComplete="off" className={`w-full p-4 bg-inputBg text-white placeholder:text-subTitle rounded-lg focus:outline-none focus:border ${errors.email ? "border-red-500" : "focus:border-primary-main"}`} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[1.875rem] mt-[1.875rem]">
            ...
            <div>
              ...
              <div className="relative" ref={genderDropdown.dropdownRef}>
                <button type="button" onClick={genderDropdown.toggleDropdown} className={`w-full p-4 bg-inputBg text-white rounded-lg focus:outline-none focus:border flex items-center justify-between ${errors.gender ? "border-red-500" : "focus:border-primary-main"}`}>
            ...
            <div>
              ...
              <div className="relative">
                <DatePicker selected={dob} onChange={(date) => dispatch(setDob(date))} dateFormat="dd/MM/yyyy" placeholderText="DD/MM/YYYY" className={`w-full p-4 bg-inputBg text-white placeholder:text-subTitle rounded-lg focus:outline-none focus:border pr-12 ${errors.dob ? "border-red-500" : "focus:border-primary-main"}`} wrapperClassName="w-full" showPopperArrow={false} popperPlacement="bottom-start" autoComplete="off" />
          ...
          <div className="mt-[1.875rem]">
            ...
            <div className="relative" ref={countryDropdown.dropdownRef}>
              <button type="button" onClick={countryDropdown.toggleDropdown} className={`w-full p-4 bg-inputBg text-white rounded-lg focus:outline-none focus:border flex items-center justify-between ${errors.country ? "border-red-500" : "focus:border-primary-main"}`}>
              ...
              {countryDropdown.isOpen && (
                <div className={`absolute flex flex-col left-0 right-0 bg-inputBg rounded-lg shadow-lg z-50 ${countryDropdown.direction === "up" ? "bottom-full mb-2" : "top-full mt-1"}`} style={{ maxHeight: 300 }}>
                  <input type="text" value={countryDropdown.search} onChange={(e) => countryDropdown.setSearch(e.target.value)} placeholder="Search country..." className="p-3 bg-transparent text-white border-b border-gray-600 outline-none" autoComplete="off" />
          ...
          {/* ReCAPTCHA Field */}
          <div className="mt-[1.875rem]">
            <ReCAPTCHA ref={recaptchaRef} sitekey={import.meta.env.VITE_RECAPTCHA_SITE_KEY} onChange={handleCaptchaChange} theme="dark" size="normal" />
            {errors.captcha && <p className="text-red-500 text-xs mt-1">{errors.captcha.message}</p>}
          </div>

          {/* Submit Button */}
          <div className="mt-[1.875rem]">
            <MagicBento textAutoHide={true} enableStars={true} enableSpotlight={true} enableBorderGlow={true} enableTilt={true} enableMagnetism={true} clickEffect={true} spotlightRadius={300} particleCount={12} glowColor="15, 239, 158" buttonText={isSubmitting ? "Submitting..." : "Submit"} width="100%" buttonType="submit" />
          </div>
        </form>
      </div>
    </div>
